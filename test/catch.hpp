...
namespace Catch {
    ...
    public:
        AssertionResult(bool passed) : m_passed(passed) {}
        bool succeeded() const { return m_passed; }
        bool failed() const { return !m_passed; }
    private:
        bool m_passed;
    };
    ...
    public:
        TestCase(const char* name, const char* description, const SourceLineInfo& lineInfo = {"", 0})
            : m_name(name), m_description(description), m_lineInfo(lineInfo) {}

        const char* getName() const { return m_name; }
        const char* getDescription() const { return m_description; }
        const SourceLineInfo& getLineInfo() const { return m_lineInfo; }
    ...
    #define SECTION(name) \
        if(Catch::Section section##__LINE__(name))
