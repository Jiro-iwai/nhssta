...
値ベースキャッシュを実装することで、理論上は最大2,836ノード（69.8%）の削減が可能ですが、これは**全ての呼び出しがキャッシュヒットする**という過度に楽観的な仮定に基づいています。

**実際の削減効果**:
- 現実的な見積もり: 20-50%の削減（約580-1,450ノード）
- 削減効果は、同じ値を持つ Expression の割合に依存

**詳細**: `docs/issue_188_value_based_cache_assumption.md` を参照
