...
protected:
    ...
    double measureExecutionTime(const std::string& dlib, const std::string& bench, 
        ...
        std::string cmd = nhssta_path + " " + options + " -d " + example_dir + "/" + dlib +
                         " -b " + example_dir + "/" + bench + " > /dev/null 2>&1";
...
TEST_F(PerformanceTest, BenchmarkS298) {
    ...
    std::cout << "\n[PERF] gaussdelay.dlib + s298.bench: " 
              << std::fixed << std::setprecision(2) << time_ms << " ms" << std::endl;
...
TEST_F(PerformanceTest, BenchmarkS344) {
    ...
    std::cout << "\n[PERF] gaussdelay.dlib + s344.bench: " 
              << std::fixed << std::setprecision(2) << time_ms << " ms" << std::endl;
...
TEST_F(PerformanceTest, BenchmarkS820) {
    ...
    std::cout << "\n[PERF] gaussdelay.dlib + s820.bench: " 
              << std::fixed << std::setprecision(2) << time_ms << " ms" << std::endl;
...
TEST_F(PerformanceTest, BenchmarkLatVsCorrelation) {
    ...
    std::cout << "\n[PERF] LAT only: " << std::fixed << std::setprecision(2) << time_lat << " ms" << std::endl;
    std::cout << "[PERF] LAT + Correlation: " << std::fixed << std::setprecision(2) << time_both << " ms" << std::endl;
    ...
    if (time_lat > 0.0) {
        ...
        std::cout << "[PERF] Correlation/LAT ratio: " << std::fixed << std::setprecision(2) 
                  << ratio << std::endl;
    }
}
...
TEST_F(PerformanceTest, BenchmarkConsistency) {
    ...
    std::cout << "\n[PERF] Average execution time: " << std::fixed << std::setprecision(2) 
              << avg << " ms" << std::endl;
    std::cout << "[PERF] Max deviation: " << std::fixed << std::setprecision(2) 
              << max_diff << " ms" << std::endl;
