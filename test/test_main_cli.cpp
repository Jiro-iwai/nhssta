...
protected:
    ...
    struct Argv {
        ...
        int argc() const { return static_cast<int>(args.size() - 1); }
...
TEST_F(CommandLineTest, TestHelpShort) {
    ...
    EXPECT_NE(output.find("-d"), std::string::npos) 
        << "Help output should contain '-d' option";
    EXPECT_NE(output.find("-b"), std::string::npos) 
        << "Help output should contain '-b' option";
}
...
TEST_F(CommandLineTest, TestDlibLong) {
    std::string output = run_nhssta({"--dlib", "../example/my.dlib", "--bench", "../example/my.bench"});
    EXPECT_EQ(output.find("usage"), std::string::npos) 
        << "Should not show usage when valid options provided";
}
...
TEST_F(CommandLineTest, TestLatShort) {
    std::string output = run_nhssta({"-l", "-d", "../example/my.dlib", "-b", "../example/my.bench"});
    EXPECT_EQ(output.find("usage"), std::string::npos) 
        << "Should not show usage when valid options provided";
}
...
TEST_F(CommandLineTest, TestLatLong) {
    std::string output = run_nhssta({"--lat", "-d", "../example/my.dlib", "-b", "../example/my.bench"});
    EXPECT_EQ(output.find("usage"), std::string::npos) 
        << "Should not show usage when valid options provided";
}
...
TEST_F(CommandLineTest, TestCorrelationShort) {
    std::string output = run_nhssta({"-c", "-d", "../example/my.dlib", "-b", "../example/my.bench"});
    EXPECT_EQ(output.find("usage"), std::string::npos) 
        << "Should not show usage when valid options provided";
}
...
TEST_F(CommandLineTest, TestCorrelationLong) {
    std::string output = run_nhssta({"--correlation", "-d", "../example/my.dlib", "-b", "../example/my.bench"});
    EXPECT_EQ(output.find("usage"), std::string::npos) 
        << "Should not show usage when valid options provided";
}
...
TEST_F(CommandLineTest, TestMultipleOptions) {
    std::string output = run_nhssta({"-l", "-c", "-d", "../example/my.dlib", "-b", "../example/my.bench"});
    EXPECT_EQ(output.find("usage"), std::string::npos) 
        << "Should not show usage when valid options provided";
}
...
TEST_F(CommandLineTest, TestInvalidOption) {
    std::string output = run_nhssta({"-x", "-d", "../example/my.dlib", "-b", "../example/my.bench"});
    // Should show usage or error
    EXPECT_NE(output.find("usage"), std::string::npos) 
        << "Should show usage for invalid option";
}
...
TEST_F(CommandLineTest, TestEmptyArguments) {
    ...
    EXPECT_TRUE(has_error) 
        << "Should show error or usage when no arguments provided. Got: " << output.substr(0, 200);
}
