...
CriticalPaths Ssta::getCriticalPaths(size_t top_n) const {
    ...
    if (!is_critical_path_ || outputs_.empty()) {
        return paths;
    }
    ...
    build_path = [&](const std::string& signal_name, 
        ...
        if (inputs_.find(signal_name) != inputs_.end()) {
            // Reached input node, create path
            // node_path[0] is the output node (first added), node_path[last] is input node
            ...
            // Use the output LAT value passed as parameter
            paths.emplace_back(reversed_nodes, reversed_instances, output_lat);
            node_path.pop_back();
            return;
        }
        ...
        if (inst_it == signal_to_instance_.end()) {
            // No instance found - this might be an input node that wasn't tracked
            // or a signal that wasn't generated by an instance
            // If it's not an input, we can't build a path, so return
            if (inputs_.find(signal_name) == inputs_.end()) {
                node_path.pop_back();
                return;
            }
            // It's an input node, create path
            std::vector<std::string> reversed_nodes(node_path.rbegin(), node_path.rend());
            std::vector<std::string> reversed_instances(instance_path.rbegin(), instance_path.rend());
            // Use the output LAT value passed as parameter
            paths.emplace_back(reversed_nodes, reversed_instances, output_lat);
